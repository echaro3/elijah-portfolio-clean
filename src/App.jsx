...
export default function ElijahCharoPortfolio() {
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white font-sans overflow-hidden">
      <section className="relative flex items-center justify-center px-4 py-8 sm:px-6 sm:py-12 lg:min-h-screen lg:py-16">
        ...
        <div className="relative z-10 w-full max-w-7xl grid gap-5 sm:gap-6 lg:grid-cols-[1.35fr_0.9fr] lg:gap-12 lg:items-center">
          <section className="rounded-3xl border border-white/10 bg-slate-950/60 p-5 sm:p-6 md:p-8 backdrop-blur-xl shadow-2xl">
            <div className="inline-flex max-w-full items-center gap-2 rounded-full border border-blue-400/30 bg-blue-400/10 px-3 py-2 text-xs sm:text-sm text-blue-200 mb-4 sm:mb-5">
              <span className="h-2 w-2 shrink-0 rounded-full bg-blue-400 animate-pulse" />
              <span className="truncate">San Antonio · Data Engineering · Automation · Analytics</span>
            </div>
            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black tracking-tight leading-tight lg:leading-[0.98]">
              I build systems that turn <span className="block text-transparent bg-clip-text bg-gradient-to-r from-blue-300 via-cyan-200 to-blue-500">manual work into momentum.</span>
            </h1>
            <p className="mt-4 sm:mt-5 text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl leading-relaxed">
            ...
            <div className="mt-6 sm:mt-7 grid gap-3 sm:flex sm:flex-wrap sm:gap-4">
              <a href="mailto:elijahcharo285@gmail.com" className="w-full sm:w-auto text-center rounded-full bg-blue-500 px-5 py-3 text-sm sm:text-base font-semibold text-white shadow-[0_0_24px_rgba(59,130,246,0.28)] hover:bg-blue-400 transition">Let’s Build Something</a>
              <a href="/ElijahCharo_Data_Engineering_Resume.pdf" target="_blank" rel="noopener noreferrer" className="w-full sm:w-auto text-center rounded-full border border-slate-600 px-5 py-3 text-sm sm:text-base font-semibold text-slate-100 hover:border-blue-400 hover:text-blue-300 transition">Download Resume</a>
            </div>
            <div className="mt-6 sm:mt-7 grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 max-w-3xl">
              <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4"><p className="text-2xl sm:text-3xl font-bold text-blue-300">300+</p><p className="text-xs sm:text-sm text-slate-400">SharePoint sites supported</p></div>
              <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4"><p className="text-2xl sm:text-3xl font-bold text-blue-300">TS/SCI</p><p className="text-xs sm:text-sm text-slate-400">Clearance</p></div>
              <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4"><p className="text-2xl sm:text-3xl font-bold text-blue-300">40K+</p><p className="text-xs sm:text-sm text-slate-400">Personnel-supported environment</p></div>
            </div>
          </section>

          <aside className="relative">
            <div className="absolute -inset-3 rounded-3xl bg-blue-500/20 blur-3xl" />
            <div className="relative rounded-3xl border border-white/10 bg-slate-950/70 p-5 sm:p-7 lg:p-8 backdrop-blur-xl shadow-2xl">
              <div className="flex flex-col items-center text-center">
                <img src="/elijah-charo-power-bi-sharepoint-automation-specialist-san-antonio.jpg" alt="Elijah Charo - Data Engineer, Power BI, SharePoint, and Automation Specialist in San Antonio" width="112" height="112" loading="eager" className="h-20 w-20 sm:h-24 sm:w-24 rounded-full object-cover object-top border-2 border-blue-400 shadow-[0_0_18px_rgba(96,165,250,0.40)]" />
                <h2 className="mt-4 sm:mt-5 text-2xl sm:text-3xl font-bold">Elijah Charo</h2>
                <p className="mt-2 text-sm sm:text-base text-blue-200">Data Engineer | Automation & Operations</p>
                <p className="mt-3 sm:mt-4 text-xs sm:text-sm text-slate-400 leading-relaxed">Data Engineering · Power BI · Power Apps · Power Automate · SharePoint · Microsoft 365 · Analytics · Operations Modernization</p>
              ...
              <div className="mt-6 sm:mt-8 grid grid-cols-1 min-[420px]:grid-cols-2 gap-3">
                {skills.map((skill) => <div key={skill} className="rounded-xl border border-white/10 bg-slate-900/80 px-3 py-2 text-xs sm:text-sm text-slate-200">{skill}</div>)}
              </div>
            </div>
          </aside>
        </div>
      ...
      <section className="px-4 sm:px-6 py-14 sm:py-20 bg-slate-950">
        <div className="max-w-6xl mx-auto">
          <div className="max-w-3xl"><p className="text-blue-300 font-semibold">What I Build</p><h2 className="mt-3 text-3xl sm:text-4xl md:text-5xl font-black tracking-tight">Digital solutions for teams buried in manual processes.</h2><p className="mt-4 sm:mt-5 text-slate-400 text-base sm:text-lg leading-relaxed">My focus is simple: replace scattered spreadsheets, inbox chaos, and repetitive status tracking with data pipelines, dashboards, apps, and Microsoft-based systems that are easier to manage and easier to measure.</p></div>
          <div className="mt-8 sm:mt-12 grid md:grid-cols-2 lg:grid-cols-4 gap-5">{services.map((service) => <article key={service.title} className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 sm:p-6 hover:border-blue-400/50 hover:bg-blue-400/[0.04] transition"><h3 className="text-lg sm:text-xl font-bold text-white">{service.title}</h3><p className="mt-3 text-sm sm:text-base text-slate-400 leading-relaxed">{service.text}</p></article>)}</div>
        </div>
      </section>
